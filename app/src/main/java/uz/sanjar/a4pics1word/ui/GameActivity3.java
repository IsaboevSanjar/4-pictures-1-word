package uz.sanjar.a4pics1word.ui;
...
import uz.sanjar.a4pics1word.R;
...
import uz.sanjar.a4pics1word.core.manager.DataLoader3;
import uz.sanjar.a4pics1word.core.manager.GameManager3;
...
public class GameActivity3 extends AppCompatActivity {
    ...
    private LinearLayout answerGroup, endLevel3;
    private TextView deltaView, scorePlus, trueAnswer, closeButton;
    private Button currentLevelView, totalScoreView;
    private LinearLayout clearAnswer, helpButton, linearLayout, helpAndCancel;

    private DataLoader3 dataLoader;
    ...
    private void startGame() {
        if (ScoreCache3.getScoreCache().getLastScore() == 1) {
            ...
            ScoreCache3.getScoreCache().setLastScore(0);
        }
        windowMain();
        clearView();
        if (manager.hasData()) {
            ...
            MemoryHelper4.getHelper().setLastMainLevel4(true);
            endLevel3.setVisibility(View.VISIBLE);
            answerGroup.setVisibility(View.GONE);
            variantGroup.setVisibility(View.GONE);
            helpAndCancel.setVisibility(View.GONE);
            windowMainEnd();
        }
    }
    ...
    private void deleteVariantButtons() {
        helpButton.setOnClickListener(view -> {
            if (countSign == manager.getCurrentAnswerLength()) {
                Toast.makeText(GameActivity3.this, "There is no empty space!!!", Toast.LENGTH_SHORT).show();
            } else if (manager.getTotalScore() < 140) {
                Toast.makeText(GameActivity3.this, "You must have at least 140 scores", Toast.LENGTH_SHORT).show();
            } else {
                countSign++;
    ...
    private void checkHelp() {
        if (countSign == manager.getCurrentAnswerLength()) {
            ...
            ScoreCache3.getScoreCache().setLastScore(1);
...
//                Toast.makeText(this, "wrong", Toast.LENGTH_SHORT).show();
    ...
    private void loadData() {
        dataLoader = new DataLoader3(this);
        data = dataLoader.getRestData();

        manager = new GameManager3(data);

    }

    private void loadViews() {
        helpAndCancel = findViewById(R.id.help_and_clear);
        ...
        endLevel3 = findViewById(R.id.end_level3);
    }

    @Override
    protected void onStop() {
        super.onStop();
        LevelCache3.getLevelCache().setLastLevel(manager.getCurrentLevel());
        ScoreCache3.getScoreCache().setLastScore(manager.getTotalScore());
...
//                Toast.makeText(GameActivity1.this, "Bosildi", Toast.LENGTH_SHORT).show();
   ...
   /*     snackbar.addCallback(new Snackbar.Callback(){
    ...
    public void onBackPressed() {
        super.onBackPressed();
        Intent intent = new Intent(GameActivity3.this, StartActivity.class);
